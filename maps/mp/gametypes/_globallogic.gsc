...
init()
{
	...
	//precacheShader( "faction_128_gun" ); //Future Knives Vs Guns faction shaders
	//precacheShader( "faction_128_knife" ); //Future Knives Vs Guns faction shaders
...
matchStartTimer()
{
	...
	if( level.scr_match_readyup_period == 1 )
	{
		...
		game["matchReadyUpText"] setText( &"MP_READYUP_ALL_PLAYERS_READY" );
	...
	if( level.prematchPeriodEnd > 0 )
	{
		if( !game["roundsplayed"] )
			...
			matchStartText setText( &"MP_MATCH_RESUMING_IN" );
		...
		if ( level.scr_match_readyup_period == 1 )
		{
			...
			while ( gettime() < gameStarts )
			{
				...
				if( gettime() > nextSwitch )
				{
					...
					else if ( level.scr_match_readyup_public == 0 )
						game["matchReadyUpText"] setText( &"MP_READYUP_RECORD_REMINDER" ); 
...
//	if ( level.prematchPeriod > 0 )
		maps\mp\gametypes\_readyup::start();
...
//	self maps\mp\gametypes\_weapons::updateWeaponUsageStats();
	...
	if( isPlayer( attacker ) )
	{
		...
		if ( attacker == self ) // killed himself
		{
			...
			if ( isDefined( self.switching_teams ) )
			{
				if ( !level.teamBased && ((self.leaving_team == "allies" && self.joining_team == "axis") || (self.leaving_team == "axis" && self.joining_team == "allies")) )
				{
					...
					if( (playerCounts[self.joining_team] - playerCounts[self.leaving_team]) > 1 && !level.inReadyUpPeriod )
					{
			...
			if( isDefined( self.friendlydamage ) && !level.inReadyUpPeriod )
		...
		else
		{
			...
			else if ( level.teamBased && self.pers["team"] == attacker.pers["team"] && !level.inReadyUpPeriod ) // killed by a friendly
			{
			...
			else
			{
				...
				if ( level.teamBased && !level.inReadyUpPeriod )
				{
	...
	if(isDefined(eInflictor) && sWeapon != "cobra_ffar_mp" && (isSubStr(sWeapon, "cobra") || isSubStr(sWeapon, "gunship")))
	{
	...
	killCamSpawnTime = undefined;
	if(isDefined(eInflictor.killCamEnt) && isDefined(eInflictor.killCamEnt.spawnTime))
		killCamSpawnTime = eInflictor.killCamEnt.spawnTime;

	if(isDefined(eInflictor) && isDefined(eInflictor.killCamEnt) && ( sWeapon == "throwingknife_mp" ))
	{
//Backup Method
//		dokillcam = true;
//		killcamentity = eInflictor getEntityNumber();
//Fix Rotating Camera issue
	...
	if ( !self.cancelKillcam && doKillcam && level.killcam )
	{
		...
		self maps\mp\gametypes\_killcam::killcam(lpattacknum, killcamentity, killCamSpawnTime, sWeapon, postDeathDelay + deathTimeOffset, psOffsetTime, willRespawnImmediately, timeUntilRoundEnd(), perks, attacker );
	}
