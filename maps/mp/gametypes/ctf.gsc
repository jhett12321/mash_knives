...
main()
{
	 ...
	 maps\mp\gametypes\_globallogic::registerRoundLimitDvar( "ctf", 2, 1, 500 );
	 maps\mp\gametypes\_globallogic::registerRoundSwitchDvar( "ctf", 1, 0, 500 );
	 maps\mp\gametypes\_globallogic::registerScoreLimitDvar( "ctf", 0, 0, 5000 );
	 maps\mp\gametypes\_globallogic::registerTimeLimitDvar( "ctf", 20, 0, 1440 );
...
onPrecacheGameType()
{
	 ...
	 precacheString( &"MP_ENEMY_FLAG_RETURNED_BY" );
	 ...
	 precacheString( &"MP_FLAG_DROPPED" );
	 precacheString( &"MP_FLAG_DROPPED_BY" );
}
...
onPickup( player )
{
	 ...
	 if ( playerTeam != self.ownerTeam ) {
			...
			thread printAndSoundOnEveryone( playerTeam, getOtherTeam( playerTeam ), &"MP_FLAG_RECOVERED_BY", &"MP_ENEMY_FLAG_RECOVERED_BY", "mp_obj_taken", "mp_enemy_obj_taken", player );
	 }
...
onDrop( player )
{
	 ...
	 if ( isDefined( player ) ) {
			...
			thread printAndSoundOnEveryone( self.ownerTeam, getOtherTeam( self.ownerTeam ), &"MP_FLAG_DROPPED_BY", &"MP_ENEMY_FLAG_DROPPED_BY", "mp_war_objective_taken", "mp_war_objective_lost", player );

			player logString( self.ownerTeam + " flag dropped" );
	 } else {
			thread printAndSoundOnEveryone( self.ownerTeam, getOtherTeam( self.ownerTeam ), &"MP_FLAG_DROPPED", &"MP_ENEMY_FLAG_DROPPED", "mp_war_objective_taken", "mp_war_objective_lost", "" );
...
onReset( player )
{
	 ...
	 // Play the corresponding sounds for players
	 if ( isDefined( player ) ) {
			thread printAndSoundOnEveryone( self.ownerTeam, getOtherTeam( self.ownerTeam ), &"MP_FLAG_RETURNED_BY", &"MP_ENEMY_FLAG_RETURNED_BY", "mp_obj_returned", "mp_enemy_obj_returned", player );
