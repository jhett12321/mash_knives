...
//M*A*S*H Knives End
	...
	level.hardpointInforms["throwing_knives_mp"] = "mp_killstreak_throwing_knives";
	level.hardpointInforms["speed_mp"] = "mp_killstreak_speed";
	level.hardpointInforms["assassin_mp"] = "mp_killstreak_assassin";
	...
	precacheString( level.hardpointHints["throwing_knives_mp"] );   
	precacheString( level.hardpointHints["speed_mp"] );   
	precacheString( level.hardpointHints["assassin_mp"] );
	...
	precacheString( level.hardpointHints["speed_mp_not_available"] );
	...
	game["dialog"]["throwing_knives_mp"] = "";
	game["dialog"]["speed_mp"] = "speed";
	game["dialog"]["assassin_mp"] = "assassin";
...
triggerHardPoint( hardpointType )
{
	...
	else if ( hardpointType == "throwing_knives_mp" )
	{
		self thread useknifeItem();
	}
	else if ( hardpointType == "speed_mp" )
	{
		if ( (isDefined( self.isSpeed ) && self.isSpeed) || (isdefined(self.isAssassin) && self.isAssassin) )
		{
			self iPrintLnBold( level.hardpointHints[hardpointType+"_not_available"] );
			return false;
		}
		else
		self thread useSpeedItem();
	}
	else if ( hardpointType == "assassin_mp" )
	{
		if ( (isDefined( self.isSpeed ) && self.isSpeed) || (isdefined(self.isAssassin) && self.isAssassin) )
		{
			self iPrintLnBold( level.hardpointHints[hardpointType+"_not_available"] );
...
self endon("assassin_used");
	...
	while(1)
	{
		...
		if( self UseButtonPressed())
		{
			...
			if( !isdefined(self.isnotFrozen) || self.isnotFrozen )
			self thread assassin_time();
		}
	}
}
