...
isHackWeapon( weapon )
{
	if ( weapon == "radar_mp" || weapon == "airstrike_mp" || weapon == "helicopter_mp" || weapon == "throwing_knives_mp" || weapon == "speed_mp" )
		return true;
...
watchGrenadeUsage()
{
	...
	if ( getdvarint("scr_deleteexplosivesonspawn") == 1 )
	{
		...
		self.claymorearray = [];
		//delete throwing knives from previous spawn
		if ( isdefined( self.throwingknifearray ) )
		{
			for ( i = 0; i < self.throwingknifearray.size; i++ )
			{
				if ( isdefined(self.throwingknifearray[i]) )
					self.throwingknifearray[i] delete();
			}
		}
		if ( isdefined( self.throwingknifearraytriggers ) )
		{
			for ( i = 0; i < self.throwingknifearraytriggers.size; i++ )
			{
				if ( isdefined(self.throwingknifearraytriggers[i]) )
					self.throwingknifearraytriggers[i] delete();
			}
		}
	}
	else
	...
	for ( ;; )
	{
		...
		if ( weaponName == "throwingknife_mp" )
			self beginThrowingKnifeTracking();
		else
...
beginThrowingKnifeTracking()
{
	...
	self waittill ( "grenade_fire", throwingknife, weaponName );
	
	
	if ( weaponName == "throwingknife_mp" )
	{
		throwingknife.originalOwner = self;
	}
	self.throwingGrenade = false;
}
...
watchThrowingKnives(player)
{
	...
	while(1)
	{
		...
		{
			self.throwingknifearray[self.throwingknifearray.size] = throwingknife;
			throwingknife.owner = self;
			throwingknife thread maps\mp\_mashutil::createKillCamEnt(undefined, (0,0,0), (0,0,0));
			throwingknife thread knifePickup(player);
		}
	}
}
